var labclock = {
    ...
    startPhase: function () {
        document.canvas.src= "stimuli/"+this.experiment.imagesArray[Math.floor(Math.random() * this.experiment.imagesArray.length)];
        this.trialsIndex = 0;
    ...
    startTrial: function (playSound) {
        ...
        if (this.trialsIndex < this.experiment.phases[this.phasesIndex].trials.length) {
            ...
            this.experiment.messages.trialSelecting = this.experiment.phases[this.phasesIndex].messagezeiteingabe;
            this.experiment.responseKey = this.experiment.phases[this.phasesIndex].taste;
    ...
    storeExperimentData: function () {
        var results = '',
            basicResultsR1 = 'ID;Group;',
            basicResultsR2 = '\n ;' + this.experiment.code + ';',
            resultsEndR1 = 'Full Log; Date;',
            resultsEndR2 = 'Full Log;'+Date()+';',
            xhr, storageItem;
        storageItem = results;
        for (var p = 0, lp = this.experiment.phases.length; p < lp; p++) {
            for (var t = 0, lt = this.experiment.phases[p].trials.length; t < lt; t++) {

                if (this.experiment.phases[p].description != 'Übung') {
                    basicResultsR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Key;';
                    basicResultsR2 += this.experiment.phases[p].trials[t].keypressTrialTimes + ';';
           
                    basicResultsR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Guess;';
                    basicResultsR2 += this.experiment.phases[p].trials[t].guessTime + ';';

                    basicResultsR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Tone;';
                    basicResultsR2 += this.experiment.phases[p].trials[t].tone + ';';
                }
                resultsEndR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Delay;';
                resultsEndR2 += this.experiment.phases[p].trials[t].delay + ';';
                
                resultsEndR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Cycle;';
                resultsEndR2 += this.experiment.phases[p].trials[t].cycle + ';';
                
                resultsEndR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Cycle_Time;';
                resultsEndR2 += this.experiment.phases[p].trials[t].cycleTime + ';';
                
                resultsEndR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Tone;';
                resultsEndR2 += this.experiment.phases[p].trials[t].tone + ';';
                
                resultsEndR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Tone_Time;';
                resultsEndR2 += this.experiment.phases[p].trials[t].toneTime + ';';
                
                resultsEndR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Key;';
                resultsEndR2 += this.experiment.phases[p].trials[t].keypressTrialTimes + ';';
                
                resultsEndR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Start_Trial_Time;';
                resultsEndR2 += this.experiment.phases[p].trials[t].startTrialTime + ';';
                
                resultsEndR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_End_Trial_Time;';
                resultsEndR2 += this.experiment.phases[p].trials[t].endTrialTime + ';';
                
                resultsEndR1 += this.experiment.phases[p].description + '_T' + (+t + 1) + '_Start_Trial_Audio_Time;';
                resultsEndR2 += this.experiment.phases[p].trials[t].startTrialAudioTime + ';';
            }
        }
        //results += resultsEndR2;
        results = basicResultsR1 +resultsEndR1 + basicResultsR2 + resultsEndR2;
    ...
    displayState: function () {
        ...
        case this.STATE_TRIAL_READY:
            ...
            if (this.experiment.phases[this.phasesIndex].trials[this.trialsIndex].trialReady) {
                ...
                this.expScreenCaption.innerHTML = this.experiment.messages.trialReady = this.experiment.phases[this.phasesIndex].messageinstruction;
                document.canvas.style.visibility = "visible";
                this.dot.style.display = 'none';
                setTimeout( () => {document.canvas.style.visibility = "hidden";document.canvas.src= "stimuli/"+this.experiment.imagesArray[Math.floor(Math.random() * this.experiment.imagesArray.length)];this.dot.style.display = 'block';
                },  this.experiment.stimulusDelay);
            }
